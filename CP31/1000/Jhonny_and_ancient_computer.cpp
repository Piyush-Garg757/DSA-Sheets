...
For example, if x=6, in one operation it can be replaced by 12, 24, 48 or 3. Value 6 isn't divisible by 4 or 8, so there are only four variants of replacement.
...
In the fourth test case, the initial and target values are equal, so Johnny has to do 0 operations.
...
#include <bits/stdc++.h>
using namespace std;
void fn(long long a, long long b)
{
    int steps = 0;
    while (a > b)
    {
