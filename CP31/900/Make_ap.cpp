...
For example, the following sequences are AP:
[5,10,15], [3,2,1], [1,1,1], and [13,10,7].
The following sequences are not AP:
[1,2,4], [0,1,0] and [1,3,2].
...
Output
For each test case print "YES" (without quotes) if Polycarp can choose a positive integer m and multiply exactly one of the integers a, b or c by m to make [a,b,c] be an arithmetic progression.
...
int main()
{
	...
	while (t--)
	{
		long long a,b,c;
		cin>>a>>b>>c;
		long long mul = 2*b*1ll;
		if(mul>a+c)
		{
			if(((mul)-c)%a==0 || ((mul)-a)%c==0) cout<<"YES\n";
			else cout<<"NO\n";
		}
		else
		{
			if((a+c)%mul ==0) cout<<"YES\n";
			else cout<<"NO\n";
		}
	}
}
