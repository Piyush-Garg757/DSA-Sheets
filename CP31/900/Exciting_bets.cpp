...
In one move, you can perform any one of these operations. You can perform an arbitrary (possibly zero) number of moves. Determine the maximum excitement the fans can get and the minimum number of moves required to achieve it.
...
int main()
{
    ...
    while (t--)
    {
        ...
        long long max_h = abs(a - b);
        long long modulo = a % max_h;
        long long min_steps = min(modulo, max_h - modulo);
        cout << max_h << " " << min_steps << "\n";
    }
}
// TC - O(1)  SC - O(1)